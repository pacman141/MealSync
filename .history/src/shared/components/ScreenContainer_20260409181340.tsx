...
const ScreenContainer = ({ children, safeAreaTop = true, safeAreaBottom = true, type = "light" }: ScreenContainerProps) => {
    ...
    const ScreenLight = () => <View style={{...styles.container}}>{children}</View>;

    const ScreenColor = () => <View style={{...styles.container}}>{children}</View>;
    ...
    return (
        ...
        >
            {type === "light" ? <ScreenLight /> : type === "color" ? <ScreenColor /> : <ScreenLinear />}
        </View>
    );
};
